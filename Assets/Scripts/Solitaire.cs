    ...
    IEnumerator SolitaireDeal()
    {
        for (int i = 0; i < 7; i++)
        {
            ...
            foreach (string card in bottoms[i])
            {
                ...
                // Check if the prefab or parent object has been destroyed before instantiation
                if (cardPrefab == null || bottomPos[i] == null) {
                    Debug.Log($"Instantiating card at position {i}, cardPrefab null: {cardPrefab == null}, bottomPos[{i}] null: {bottomPos[i] == null}");
                    continue; // Skip this iteration if the object is null
                }
